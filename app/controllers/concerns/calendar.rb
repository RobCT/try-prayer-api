    ...
    def render_calendar_to_json(calendar_params)
      ...
      when "month"
        ...
        (start_date..end_date).each do |dd|
          calendar << {"date"=>dd.to_time(:utc).to_formatted_s(:rfc822), "day_of_week"=>dd.to_formatted_s(:day_of_week)}
        end
      ...
      when "week"
        ...
        render json: calendar.to_json, status: 200 
      when "day"
        render json: {"date"=>@date.to_time(:utc).to_formatted_s(:rfc822), "day_of_week"=>@date.to_formatted_s(:day_of_week)}, status: 200
